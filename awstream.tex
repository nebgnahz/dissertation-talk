...
\end{lstlisting}
    ...
    child {node[yn] (c1) {[1, 2, 3, 4]}}
    child {node[yn] (c2) {[0, 1, 1, 2]}}
    child {node[yn] (c3) {[0, 0, 0, 1]}};

    \node[below=0cm of c1] {no degradation};
    \node[below=0cm of c2] {k = 2};
    \node[below=0cm of c3] {k = 4};
  \end{tikzpicture}

  We rewrite the video streaming application as follows\footnote{Example code in
