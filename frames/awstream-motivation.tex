\begin{frame}[t]{Limited Network Resource}
  ...
  \alt<3>{ \textit{$\ldots$ Dropcam, a WiFi video-streaming camera and
  ...
  \alt<4>{
    ...
    \begin{figure}
      ...
      \caption{Bandwidth variations throughout the day between Amazon EC2
        sites. Similar measurements for wireless networks, broadband access
        networks and cellular networks (\hyperlink{cellular-variation}{backup
          slides}).}
    \end{figure}
  }  
...
\begin{frame}[t]{Fidelity vs.\,Freshness}
  ...
  \only<3->{\item Application-specific optimizations don't generalize}
    \only<4>{
      \begin{itemize}
